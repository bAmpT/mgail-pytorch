...
class ER(object):
    ...
    def __init__(self, memory_size, state_dim, action_dim, reward_dim, qpos_dim, qvel_dim, batch_size, history_length=1):
        ...
        self.qpos = np.random.normal(scale=0.35, size=(self.memory_size, qpos_dim)).astype(np.float32)
        self.qvel = np.random.normal(scale=0.35, size=(self.memory_size, qvel_dim)).astype(np.float32)
