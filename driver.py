...
class Driver(object):
    def __init__(self, environment: Environment):
        ...
        self.log_name = "./logs/results_" + time.strftime("%Y-%m-%d-%H-%M") + ".log" 
    ...
    def train_step(self) -> None:
        ...
        if self.itr == 0:
            with open('agent_trajectories/hopper_er_agent.bin', 'rb') as f:
    ...
    def save_model(self, dir_name: str = None):
        ...
        if not os.path.isdir(dir_name):
            os.mkdir(dir_name)
        fname = dir_name + time.strftime("%Y-%m-%d-%H-%M-") + ('%0.6d.pth' % self.itr)
