...
class Environment(object):
    ...
    def _step(self, action: np.ndarray):
        ...
        if self.random_initialization:
            self.qpos, self.qvel = self.gym.unwrapped.data.qpos.flatten(), self.gym.unwrapped.data.qvel.flatten()
    ...
    def step(self, action: np.ndarray, mode: str):
        ...
        if mode == 'tensorflow':
            if self.random_initialization:
                state, reward, done, qval, qpos = self._step(action)
                #tf.py_func(self._step, inp=[action], Tout=[tf.float32, tf.float32, tf.bool, tf.float32, tf.float32], name='env_step_func')
            else:
                state, reward, done = self._step(action)
                #tf.py_func(self._step, inp=[action], Tout=[tf.float32, tf.float32, tf.bool],name='env_step_func')
    ...
    def _train_params(self):
        self.trained_model = 'snapshots/2022-04-22-18-52-039000.pth'
